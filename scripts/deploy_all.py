                ...
                for pat in exclude_patterns:
                    if pat.endswith('/'):
                        if pat in rel_str + '/':
                            skip = True
                            break
                    elif Path(rel_str).match(pat):
                        skip = True
                        break
                if not skip:
                    zf.write(f, f"AI-IDP-AegisTrace-2.0.0/{rel_str}")
...
def zenodo_create(token):
    ...
    # Extract abstract from paper/main.tex
    tex_content = (ROOT / "paper" / "main.tex").read_text(encoding="utf-8")
    abstract = ""
    if "\\begin{abstract}" in tex_content:
        abstract = tex_content.split("\\begin{abstract}")[1].split("\\end{abstract}")[0].strip()
    ...
    data = {
        "metadata": {
            ...
            "keywords": [
                "AI governance", "AI accountability", "AI identity", "provenance", 
                "traceability", "audit", "permanent record", "delegation", 
                "authorization", "Canadian law", "PIPEDA", "AIDA", "Ed25519", 
                "hash chain", "Merkle tree", "registry", "federation", 
                "privacy", "human rights", "Indigenous data sovereignty", "OCAP", "TRC Calls to Action"
            ],
            "access_right": "open",
            "license": "cc-by-4.0",
    ...
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    r = requests.post(f"https://zenodo.org/api/deposit/depositions", headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"}, json=data)
    r.raise_for_status()
    dep = r.json()
...
def zenodo_upload_files(dep_id, files_url, token, file_paths):
    ...
    for fp in file_paths:
        with open(fp, 'rb') as f:
            ...
            r = requests.post(files_url, headers={"Authorization": f"Bearer {token}"}, data=data, files={"file": f})
            r.raise_for_status()
...
def update_metadata(doi):
    # Update CITATION.cff
    cit_path = ROOT / "CITATION.cff"
    if cit_path.exists():
        cit = cit_path.read_text(encoding="utf-8")
        if "doi: " in cit:
            import re
            cit = re.sub(r'doi: .+', f'doi: {doi}', cit)
            cit_path.write_text(cit, encoding="utf-8")
    ...
    if tex_path.exists():
        ...
        if "\\date{" in tex and "Zenodo DOI" not in tex:
...
def main():
    ...
    if not github_token:
        ...
        print("   Set: $env:GITHUB_TOKEN='ghp_xxx'")
        return 1
    if not zenodo_token:
        print("❌ ZENODO_TOKEN not set")
        print("   Set: $env:ZENODO_TOKEN='xxx'")
        return 1
